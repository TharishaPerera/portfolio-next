...
const MovingImg = ({title, img, link}) => {
    ...
    return(
        <Link href={link} target='_blank' onMouseMove={handleMouse} onMouseLeave={handleMouseLeave} >
            <h2 className='capitalize text-xl font-semibold hover:underline underline-offset-4'>{title}</h2>
            <FramerImage style={{x:x, y:y}} initial={{opacity:0}} whileInView={{opacity:1, transition:{duration:0.3}}} ref={imgRef} src={img} alt={title} className='z-10 w-96 h-auto hidden absolute rounded-lg' />
        </Link>
    )
}
...
const BlogPost = ({ img, title, date, link }) => { 
    return (
        <motion.li initial={{y:200, opacity:0}} whileInView={{y:0, opacity:1, transition:{duration:1, ease:"easeInOut"}}} className='relative w-full p-4 py-6 my-4 rounded-xl flex items-center justify-between bg-light text-dark first:mt-0 border border-solid border-dark border-r-4 border-b-4 dark:bg-dark dark:text-light dark:border-light'>
            <MovingImg
                title={title}
                img={img} 
                link={link} 
...
const FeaturedBlog = ({ img, title, time, summary, link }) => {
    return (
        <li className='relative col-span-1 w-full p-4 bg-light border border-solid border-dark/25 rounded-2xl dark:bg-dark dark:border-light'>
            ...
            <span className='w-ful text-primary font-semibold flex items-end justify-end mt-4 dark:text-primaryDark'>{time}</span>
        </li>
    )
}
